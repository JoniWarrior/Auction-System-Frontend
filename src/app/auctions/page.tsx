...
export default function AuctionsPage() {
  ...
  const filteredAuctions = auctions.filter(auction => {
    ...
    const matchesSearch = auction.item.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const currentAuctions = filteredAuctions.sort((auc_1, auc_2) => Number(auc_1.status === "finished") - Number(auc_2.status === "finished"))
      ...
      {filteredAuctions.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {currentAuctions.map((auction) => (
            <div key={auction.id} className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300 flex flex-col">
              ...
              <div className="p-4 flex flex-col flex-grow">
                ...
                <div className="flex justify-between items-center mb-3">
                  <div>
                    ...
                    <span className="text-2xl font-bold text-purple-700 block">${auction.current_price.toLocaleString()}</span>
                  </div>
                </div>
                ...
                <div className="flex justify-center mt-4 mb-4">
                  <p className="text-sm text-gray-700 border border-black rounded-lg px-4 py-2 bg-white shadow-md font-medium">
                    Winning Bid: <span className="text-purple-700 font-bold">${auction?.winningBid?.amount ?? auction.starting_price}</span>
                  </p>
                </div>
                ...
                <Link 
                  ...
                  {(auction.status === 'finished' || user?.role === "seller") ? 'View Results' : 'Place Bid'}
                </Link>
              </div>
            </div>
          ))}
